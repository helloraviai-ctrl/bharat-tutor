...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (PROVIDER === "gemini") {
      ...
      if (!key) return { statusCode: 500, headers: HEADERS, body: JSON.stringify({ error: "GEMINI_API_KEY missing" }) };
    ...
    if (PROVIDER === "cohere") {
      ...
      if (!key) return { statusCode: 500, headers: HEADERS, body: JSON.stringify({ error: "COHERE_API_KEY missing" }) };
